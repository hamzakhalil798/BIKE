...
def init_distributed_mode(args):
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        args.rank = 0
        args.world_size = 1
        args.gpu = 0  # Set default GPU if needed (can be changed based on your setup)
        args.dist_backend = 'nccl'  # This is typically set for CUDA even if not used
        args.dist_url = 'env://'
...
def ddp_all_gather(*args):
    """All gather by DDP. All gather doesn't have grad_fn by default."""
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        world_size = 1  # No distributed setup
    ...
    for x in args:
        if world_size > 1:
            ...
            # If no DDP, simply return the input as the gathered output
            ret = [x]
